...
import { Plus, CheckSquare, Loader2, Settings } from "lucide-react";
...
function OpportunitiesContent() {
  const [selectedPipeline, setSelectedPipeline] = useState<string>("Main Lead Flow");
  ...
  const pipelines = useQuery(api.pipelineStages.listPipelines);
  ...
  useEffect(() => {
    if (pipelines && pipelines.length > 0 && !pipelines.find(p => p.name === selectedPipeline)) {
  ...
  // Loading state
  const isLoading = pipelines === undefined || stages === undefined || opportunitiesData === undefined;
          ...
          <SelectContent>
            {pipelines?.map((pipeline) => (
              <SelectItem key={pipeline.name} value={pipeline.name}>
                {pipeline.name} ({pipeline.stageCount})
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
